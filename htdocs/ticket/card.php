		...
		if ($action != 'presend' && $action != 'presend_addmessage' && $action != 'editline') {
			...
			if (empty($reshook))
			{
				...
				if ($user->rights->ticket->write && ! $user->societe_id) {
					print '<div class="inline-block divButAction"><a class="butAction" href="card.php?clone_id=' . $object->id . '&action=create&projectid='. $object->fk_project .'">' . $langs->trans('CloneTicket') . '</a></div>';
				}
				// Fin Spé
